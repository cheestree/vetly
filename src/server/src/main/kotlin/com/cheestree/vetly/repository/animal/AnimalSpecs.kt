...
object AnimalSpecs {
    ...
    fun sexEquals(sex: Sex?) = Specification<Animal> { root, _, cb ->
        sex?.let { cb.equal(root.get<Sex>("sex"), it) } ?: cb.conjunction()
    }

    fun isSterile(sterile: Boolean?) = Specification<Animal> { root, _, cb ->
    ...
    fun bornIn(from: LocalDate?, to: LocalDate?) = BaseSpecs.betweenDates<Animal>(
        from,
        to,
        "birthDate"
    )

    fun hasOwner(owned: Boolean?) = Specification<Animal> { root, _, cb ->
        owned?.let {
            if (it) cb.isNotNull(root.get<User>("owner"))
            else cb.isNull(root.get<User>("owner"))
        } ?: cb.conjunction()
    }

    fun isSelf(self: Boolean?, userId: Long) = Specification<Animal> { root, _, cb ->
        self?.let {
    ...
    fun byEmail(userEmail: String?, authRoles: Set<Role>, authEmail: String) = Specification<Animal> { root, _, cb ->
    ...
    fun isActive(active: Boolean?, authRoles: Set<Role>) = Specification<Animal> { root, _, cb ->
