...
sealed class VetException(
    ...
    class InactiveResourceException(resourceType: ResourceType, resourceId: Any) :
        VetException("$resourceType with id $resourceId is not active")
    class ForbiddenException(message: String): VetException(message)

    //  Resource state issues
    class ResourceNotFoundException(resourceType: ResourceType, resourceId: Any) :
        VetException("$resourceType with id $resourceId not found")
    class ResourceAlreadyExistsException(resourceType: ResourceType, identifier: String, value: Any) :
        VetException("$resourceType with $identifier $value already exists")

    //  Input validation
    class ValidationException(message: String) : VetException(message)
    ...
    //  Conflicts
    class ConflictException(message: String) : VetException(message)
