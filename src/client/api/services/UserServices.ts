...
async function getUser(userId: string): Promise<UserInformation> {
  ...
  return fetch(ApiPaths.root + ApiPaths.users.get(userId), buildFetchOptions("GET", undefined, idToken))
...
async function getUserProfile(): Promise<UserInformation> {
  ...
  return fetch(ApiPaths.root + ApiPaths.users.get_user_profile(), buildFetchOptions("GET", undefined, idToken))
    .then((response) => response.json())
    .catch((error) => {
